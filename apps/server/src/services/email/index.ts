...
// Check if email is configured (any of the providers)
const hasEmailConfig = 
  (Bun.env.EMAIL_HOST && Bun.env.EMAIL_PORT) || // nodemailer
  Bun.env.RESEND_API_KEY // resend

async function sendEmail(payload: SendEmailOptions) {
  const { to, subject, html } = payload

  if (!hasEmailConfig) {
    // Log email instead of sending (email not configured)
    console.log(`[Email disabled] To: ${to}, Subject: ${subject}`)
    console.log(`[Email disabled] HTML: ${html.substring(0, 100)}...`)
    return
  }

  if (Bun.env.RESEND_API_KEY) {
    // Use Resend if available (deprecated, will be removed)
    const { sendEmail: resendSendEmail } = await import('./resend')
